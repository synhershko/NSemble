...
namespace NSemble.Web.Core
{
    ...
    public class NSembleRouteResolver : IRouteResolver, IDiagnosticsProvider
    {
        ...
        public NSembleRouteResolver(INancyModuleCatalog nancyModuleCatalog, IRoutePatternMatcher routePatternMatcher,
                                    INancyModuleBuilder moduleBuilder, IRouteCache cache,
        ...
        public ResolveResult Resolve(NancyContext context)
        {
            ...
            // First, try resolving the admin area. By convention, it is always "admin/moduleName/path", where "admin" is configurable,
            // and the admin screens should be able to handle more than one front-end area using the module
            if (context.Request.Path.StartsWith(AreasResolver.Instance.AdminAreaPrefix))
            {
                ...
                // TODO: use path to get all the AreaConfigs for areas using this module, and pass them on to the admin module
                var ret = Resolve(string.Concat(Constants.ResolverAdminAreaPrefix, path), context, this.cache);
                return ret.Selected;
            }
            ...
            var result = this.Resolve(newPath, context, this.cache);
        ...
        private ResolveResult CreateRouteAndParametersFromMatch(NancyContext context, RouteCandidate routeMatchToReturn)
        {
            ...
            return new ResolveResult(route, routeMatchToReturn.Item4.Parameters, associatedModule.Before,
                                     associatedModule.After, associatedModule.OnError);
        }
        ...
        private static IEnumerable<RouteCandidate> GetTopRouteMatchesNew(
            Tuple<List<RouteCandidate>, Dictionary<string, List<RouteCandidate>>> routes)
        {
            var maxSegments = 0;
            var maxParameters = 0;
        ...
        private ResolveResults Resolve(string path, NancyContext context, IRouteCache routeCache)
        {
            if (routeCache.IsEmpty())
            {
                ...
                return new ResolveResults
                           {
                               Selected =
                                   new ResolveResult(new NotFoundRoute(context.Request.Method, path),
                                                     DynamicDictionary.Empty, null, null)
                           };
            }
            ...
            routes =
                routes.Filter(context, "Invalid condition", (ctx, route) =>
                                                                {
                                                                    var validCondition =
                                                                        ((route.Item3.Condition == null) ||
                                                                         (route.Item3.Condition(ctx)));
            ...
            if (!routes.Item1.Any())
            {
                context.Trace.TraceLog.WriteLog(
                    s => s.AppendLine("[DefaultRouteResolver] No route had a valid condition"));
                return new ResolveResults
                           {
                               Selected =
                                   new ResolveResult(new NotFoundRoute(context.Request.Method, path),
                ...
                routes.Filter(context, "Path did not match", (ctx, route) =>
                                                                 {
                                                                     var validationResult =
                                                                         this.routePatternMatcher.Match(path,
                                                                                                        route.Item3.Path,
                                                                                                        route.Item3.
                                                                                                            Segments,
                                                                                                        context);
                                                                     ...
                                                                     var routeToReturn =
                                                                         (validationResult.IsMatch)
                                                                             ? new RouteCandidate(route.Item1,
                                                                                                  route.Item2,
                                                                                                  route.Item3,
                                                                                                  validationResult)
                                                                             : route;
            ...
            {
                context.Trace.TraceLog.WriteLog(
                    s => s.AppendLine("[DefaultRouteResolver] No route matched the requested path"));
                return new ResolveResults
                           {
                               Selected =
                                   new ResolveResult(new NotFoundRoute(context.Request.Method, path),
                ...
                routes.Filter(context, "Request method did not match", (ctx, route) =>
                                                                           {
                                                                               ...
                                                                               var methodIsValid =
                                                                                   routeMethod.Equals(requestMethod) ||
                                                                                   (routeMethod.Equals("GET") &&
            ...
            if (!routes.Item1.Any())
            {
                ...
                if (context.Request.Method.Equals("OPTIONS"))
                {
                    return new ResolveResults
                               {
                                   Selected =
                                       new ResolveResult(new OptionsRoute(context.Request.Path, allowedMethods),
                                                         DynamicDictionary.Empty, null, null),
                                   Rejected = routes.Item2
                               };
                }
                context.Trace.TraceLog.WriteLog(
                    s => s.AppendLine("[DefaultRouteResolver] Route Matched But Method Not Allowed"));
                return new ResolveResults
                           {
                               Selected =
                                   new ResolveResult(
                                   new MethodNotAllowedRoute(path, context.Request.Method, allowedMethods),
                                   DynamicDictionary.Empty, null, null),
                               Rejected = routes.Item2
                           };
            }
            ...
            if (exactMatchResults.Item1.Any())
            {
                ...
                return new ResolveResults
                           {
                               Selected =
                                   this.CreateRouteAndParametersFromMatch(context, exactMatchResults.Item1.First()),
        ...
        {
            get { return new NSembleRouteResolverDiagnosticsProvider(this); }
        }

        public class NSembleRouteResolverDiagnosticsProvider
        {
            private readonly NSembleRouteResolver resolver;

            public NSembleRouteResolverDiagnosticsProvider(NSembleRouteResolver resolver)
            {
